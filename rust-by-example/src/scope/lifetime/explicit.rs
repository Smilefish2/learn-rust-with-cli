...
pub const NAME: &'static str = "rust-by-example:scope/lifetime/explicit";
const ABOUT: &'static str = "https://rustwiki.org/zh-CN/rust-by-example/scope/lifetime/explicit.html";
...
可以用撇号 显式地标出生命周期，语法如下：
    ...
    // `foo` 带有一个生命周期参数 `'a`
和闭包类似，使用生命周期需要泛型。
另外这个生命周期的语法也表明 了 foo 的生命周期不能超出 'a 的周期。
若要给类型显式地标注生命周期，其语法 会像是 &'a T 这样，其中 'a 的作用刚刚已经介绍了。
    foo<'a, 'b>
    // `foo` 带有生命周期参数 `'a` 和 `'b`
