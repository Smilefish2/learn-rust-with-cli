...
pub const NAME: &'static str = "rust-by-example:trait/dyn";
const ABOUT: &'static str = "https://rustwiki.org/zh-CN/rust-by-example/trait/dyn.html";
...
但是，有一个简单的解决方法。相比于直接返回一个 trait 对象，我们的函数返回一个包含一些 Animal 的 Box。
box 只是对堆中某些内存的引用。因为引用的大小是静态已知的，并且编译器可以保证引用指向已分配的堆 Animal，所以我们可以从函数中返回 trait！
