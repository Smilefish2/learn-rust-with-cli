...
pub const NAME: &'static str = "rust-by-example:fn/closures/closure_examples/iter_any";
const ABOUT: &'static str = "https://rustwiki.org/zh-CN/rust-by-example/fn/closures/closure_examples/iter_any.html";
...
/**
Iterator::any
    Iterator::any 是一个函数，若传给它一个迭代器（iterator），当其中任一元素满足 谓词（predicate）时它将返回 true，否则返回 false
（译注：谓词是闭包规定 的， true/false 是闭包作用在元素上的返回值）。它的签名如下：
